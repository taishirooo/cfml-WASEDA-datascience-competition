...
def aggregate_simulation_results2(
    estimated_policy_value_list: list,
    policy_value: float,
    experiment_config_name: str,
    experiment_config_value: int,
...
class GradientBasedPolicyDataset_dr(torch.utils.data.Dataset):
    ...
    reward: np.ndarray
    pscore: np.ndarray 
    a_mat: np.ndarray  ##変更点
    r_mat: np.ndarray  ##変更点
    q1_hat: np.ndarray  ##変更点
    q0_hat: np.ndarray  ##変更点
    #pi_0: np.ndarray
    ...
    def __post_init__(self):
        ...
        assert (
            ...
            == self.reward.shape[0]
            == self.pscore.shape[0]
            == self.a_mat.shape[0] ##変更点
            == self.r_mat.shape[0] ##変更点
            == self.q1_hat.shape[0]
            == self.q0_hat.shape[0]
            #== self.pi_0.shape[0]
        )
